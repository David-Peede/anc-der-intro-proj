...
def ooa_arc_no_intro_genotype_matrix_bootstrap(
    genotype_matrix,
    variable_positions,
    n_replicates,
):
...
simulated_genotype_matrix = np.loadtxt(
    './sim_outputs/ooa_arc_no_intro/geno_mats/rep_id_{0}_geno_mat.csv.gz'.format(rep_id),
    dtype=int, delimiter=',',
)
simulated_variable_positions = np.loadtxt(
    './sim_outputs/ooa_arc_no_intro/var_pos/rep_id_{0}_var_pos.csv.gz'.format(rep_id),
    delimiter=',',
)

# Perform bootstrapping.
bs_ceu, bs_chb, = ooa_arc_no_intro_genotype_matrix_bootstrap(simulated_genotype_matrix, simulated_variable_positions, 1000)

# Save each bootstrapped distribution as a gzipped csv.
results_prefix = './sim_outputs/ooa_arc_no_intro/bootstraps/rep_id_{0}_'.format(rep_id)
