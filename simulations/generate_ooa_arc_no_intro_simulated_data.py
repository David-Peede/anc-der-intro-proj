...
def run_100mb_ooa_arc_no_intro_sim(seed):
    # Load the demes graph.
    ooa_arc_no_intro_graph = demes.load('./HomSap__OutOfAfricaArchaicAdmixture_5R19_modified.yaml')
    ...
    ts = msprime.sim_ancestry(
        ...
        demography=msprime.Demography.from_demes(ooa_arc_no_intro_graph),
    ...
    np.savetxt(
        './sim_outputs/ooa_arc_no_intro/geno_mats/rep_id_{0}_geno_mat.csv.gz'.format(seed),
        genotype_matrix,
        fmt='%d',
        delimiter=',',
        )
    ...
    np.savetxt(
        './sim_outputs/ooa_arc_no_intro/var_pos/rep_id_{0}_var_pos.csv.gz'.format(seed),
        [variable_positions],
        fmt='%1.15f',
        delimiter=',',
        newline='\n',
    ...
    mts.dump('./sim_outputs/ooa_arc_no_intro/mut_tree_seq/rep_id_{0}_mut_tree_seq.ts'.format(seed))
    return
...
# Run the simulation!
run_100mb_ooa_arc_no_intro_sim(seed=rep_id)